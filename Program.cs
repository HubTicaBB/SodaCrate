    ...
    public void AddSoda()               // Metod för att lägga till en läsk
        {
            if (AntalFlaskor < 25)          // Om läskbacken inte är full, fråga efter namn, pris och typ
            {
                ...
                foreach (Soda flaska in minBack)    // För att kontrollera om samma drycken redan finns i backen, dvs. om pris är typ är bekanta
                {
                ...
                minBack[AntalFlaskor] = new Soda(name, price, type);    // Lagra objekt i vektorn i första tomma positionen
                Console.WriteLine("----------------------------------------");
        ...
        public void PrintCrate()    // Metod för att skriva ut läskbackens innehål
        {
            ...
            Console.WriteLine("-------------------------------------");
            foreach (Soda flaska in minBack)    // Loopa genom vektorn minBack
            {
        ...
        public void SortPrice() // Metod för att sortera drycker efter pris
        {
            ...
            for (int i = 0; i < max; i++)       // För att komma åt varje vektorns position som innehåller ett objekt
            {
                ...
                {
                    if (minBack[j].GetPrice() > minBack[j + 1].GetPrice())  // Om ett objekts pris är högre än nästa objektets pris
                    {
                        temp = minBack[j];                                  // Byta position på de två objekten    
                        minBack[j] = minBack[j + 1];
                        minBack[j + 1] = temp;
                    }
                }
            }
        ...
        public void SortName()  // Metod för att sortera drycker efter namn efter bokstavsordning
        {
            ...
            for (int i = 0; i < max; i++)       // För att komma åt varje vektorns position som innehåller ett objekt
            {
                ...
                for (int j = 0; j < nrLeft; j++)
                {
                    comp = minBack[j].GetName().CompareTo(minBack[j + 1].GetName());    // CompareTo()-metoden jämför namn av två objekt och returnerar -1 (om t ex A står framför B), 0 (både samma) eller 1 (B står framför A)
        ...
        {                       // Metoden funkar på samma sätt som SortName()-metoden, men jämför typer av objekter                         
            ...
            for (int i = 0; i < max; i++)
            {
                ...
                for (int j = 0; j < nrLeft; j++)
                {
                    comp = minBack[j].GetTyp().CompareTo(minBack[j + 1].GetTyp());
                    if (comp == 1)
                    {
                        temp = minBack[j];
                        minBack[j] = minBack[j + 1];
        ...
        public void TakeSoda()
        {
            if (AntalFlaskor > 0)
            {
                ...
                do
                {
                    ...
                    Console.WriteLine("----------------------------------");
                    for (int i = 0; i <= minBack.Length - 1; i++)
                    {
                        if (minBack[i] != null)
                        {
                            if (minBack[i].GetName() == take)
                            {
                                Console.WriteLine("En {0} är tagen ut", minBack[i].GetName());
                                minBack[i] = null;
                                AntalFlaskor--;
                                taken = true;
                                break;
                ...
                for (int i = 0; i <= minBack.Length - 1; i++)
                {
                    if (minBack[i] == null && minBack[i++] != null)
                    {
                        minBack[i] = minBack[i++];
                        minBack[i++] = null;
                    }
